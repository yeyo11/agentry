import type { Orchestration, OrchestrationSpec, OrchestrationTaskSpec, OrchestrationTaskState, TaskLimits } from './types.ts';
...
/**
 * The spec that would launch this graph as it ran: what a relaunch starts from, what a template
 * saves, and what a schedule is filled from in the UI. Read back from the orchestration, so it
 * carries the settings the graph actually ran with, including corrections made when it was resumed.
 *
 * It lives here because both sides build it: a field added to `OrchestrationSpec` and carried by one
 * copy but not the other used to show up only as "this graph did not start like the other one".
 */
...
/**
 * Whether a finished graph can start over from one of its tasks: a running or waiting graph is
 * decided task by task, a workflow runs its tasks as one session, a graph being integrated is
 * mid-merge, and one whose integration branch was already pushed would orphan the pull request.
 * `Orchestrator.rerunTask` refuses the same cases, each with a message of its own, and also the ones
 * only the server can see (a process still running, checks in flight).
 */
