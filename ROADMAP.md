...
- **Agent observability (top priority)** — see what every agent really does and step in on time:
  ...
  so workers only run typecheck and unit tests; and the gaps 0.11.0 left — a run already waiting
  when the page loads raises no notification (seed it from `pendingPrompts`), and a waiting
  notification opens the run rather than the prompt. Full plan in
