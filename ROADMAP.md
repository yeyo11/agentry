...
- **CI** — workflow in place (typecheck, tests, e2e, image build); still to do: publish the image.
...
- **Richer run control** — interactive permission prompts answered from the UI
  (`--permission-prompt-tool`), attachments/images in the composer, per-run MCP config and
  allowed-tools presets, cost budgets (`--max-budget-usd`).
- **Orchestration v2** — re-run a single task, edit and relaunch a finished graph, reusable
  templates, per-task git worktrees (`--worktree`) so parallel workers never collide.
...
- **Observability** — usage and cost over time, per project and per model; export of transcripts
  (Markdown / JSON).
- **Packaging** — multi-arch image, pinned CLI version with an in-UI update check, Helm chart /
  compose profiles, healthcheck-driven restarts.
