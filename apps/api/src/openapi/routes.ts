...
export const ROUTE_DOCS: Record<string, RouteDoc> = {
  ...
  'GET /projects/:id/sessions': d('Projects', 'Sessions under one project', { description: 'Includes those of its worktrees.', params: obj({ id: str('Project id') }), ok: list('SessionSummary') }),
  'GET /projects/loose/sessions': d('Projects', 'Sessions under no project', { description: 'Loose chats: their directory is under no imported project.', ok: list('SessionSummary') }),
  ...
  'GET /sessions/:id': d('Projects & sessions', 'A window of the transcript', { description: 'The newest `limit` entries by default — a long conversation runs to tens of megabytes, which nobody reads at once. `from` is the index the window starts at and `total` what the transcript holds, so passing the `from` of a page back as `before` reads the one before it.', querystring: obj({ sidechains: str('`1` includes subagent messages'), limit: str('Entries per page (default 200, max 1000)'), before: str('Index to read backwards from: the `from` of the previous response') }), ok: ref('SessionDetail') }),
  'GET /sessions/:id/search': d('Projects & sessions', 'Search the whole transcript', { description: 'Case-insensitive plain-text match over what the transcript view shows of each entry: text, thinking, tool names and inputs, tool results. Any run of whitespace in `q` matches any run in the text. One hit per matching entry, `index` in the same space as a page\'s `from` and `total`, so a hit on a page not loaded yet is reached by reading back to it. At most 500 hits, the newest; `truncated` says older ones were left out.', querystring: obj({ q: str('Text to find (required, up to 200 characters)'), sidechains: str('`1` includes subagent messages, as the page read with it does') }, ['q']), ok: ref('TranscriptSearchResult') }),
  ...
  'GET /runs/:id': d('Runs', 'Run summary and a window of its events', { description: 'The newest `limit` events by default, with `from` and `total` the way a session transcript reports them. To follow a run, take the last event\'s `seq` from here and open `GET /runs/:id/stream?since=` with it, which replays nothing already held.', querystring: obj({ limit: str('Events per page (default 200, max 1000)'), before: str('Index to read backwards from: the `from` of the previous response') }), ok: ref('RunDetail') }),
  'GET /runs/:id/search': d('Runs', 'Search every event of a run', { description: 'The same match as `GET /sessions/:id/search`, over what the run view shows of each event, with `index` in the space of `GET /runs/:id` pages. Each hit carries the event `kind`, so a view hiding some kinds can skip them.', querystring: obj({ q: str('Text to find (required, up to 200 characters)') }, ['q']), ok: ref('TranscriptSearchResult') }),
