...
export const ROUTE_DOCS: Record<string, RouteDoc> = {
  ...
  'GET /tasks': d('Runs', 'Background shell commands across runs and CLI sessions', { description: 'A live run reports from its stream; CLI sessions that are live and runs that have ended are read from their files, so the list survives a restart and covers sessions started from a terminal. Agents are left to `/subagents`.', ok: list('BackgroundTask') }),
