...
export default async ({ page, check }) => {
  ...
  await page.eval(`const m=[...document.querySelectorAll('main input')].find(e=>e.list&&/model/i.test(e.list.id));if(m){Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set.call(m,'haiku');m.dispatchEvent(new Event('input',{bubbles:true}));}return true`);
